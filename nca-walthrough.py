...
@app.cell(hide_code=True)
def _():
    # Define sliders — plain assignments, NO walrus operators inside layout
    ...
    # Display — bare expression, NOT in return tuple
    mo.hstack(
        [
    ...
    # Export sliders only — the layout was already displayed above
...
# ── Art helper functions (borrowed from marimo's own gallery) ────────────────

@app.function(hide_code=True)
def build_cmap(name: str, n: int) -> ListedColormap:
    """Build a ListedColormap with n colours — same palettes as marimo gallery."""
    palettes = {
        "NCA Blue-Red": ["#3060ff","#50a0ff","#60d0ff","#80eeff",
                         "#c0ffff","#fffff0","#ffee60","#ffb040","#ff6060","#ee2020"],
        "Ocean-Earth":  ["#2080ff","#00c8f0","#00f0d0","#40ffa0",
                         "#a0ff60","#ffff50","#ffc020","#ff7030","#ff4060","#f020a0"],
        "Neon":         ["#8040ff","#c040ff","#ff40ff","#ff40a0",
                         "#ff4060","#ff8020","#ffc000","#e0ff00","#40ff40","#00ffc0"],
        "Plasma":       ["#0d0887","#5302a3","#8b0aa5","#b83289",
    ...
    base = plt.get_cmap({"Viridis": "viridis", "Twilight": "twilight"}.get(name, "viridis"))
    return ListedColormap([base(i / max(n - 1, 1)) for i in range(n)])


@app.function(hide_code=True)
def smooth_grid(grid: np.ndarray, radius: int, n_states: int) -> np.ndarray:
    """Mode-filter: replace each cell with the most common state in a window."""
    if radius == 0:
        return grid
...
def blur_grid(grid: np.ndarray, sigma: float, n_states: int) -> np.ndarray:
    """Gaussian-blur each state channel then re-quantize to dominant state."""
    if sigma == 0:
        return grid
    blurred = np.stack([
...
def _(grids, n_ui):
    ...
    plt.tight_layout()
    _fig  # bare expression — displayed
    return


# ── NCA AS ART — the marimo gallery connection ───────────────────────────────
...
def _():
    ...
    The same grid dynamics that teach a language model to reason also produce
    stunning visual patterns. Use the form below to generate a high-resolution
    NCA artwork — rendered with the same colour palettes as
    [marimo's cellular automaton gallery](https://marimo.io/gallery).
    This is not decoration: it's the raw training data of a future LLM.
    """)
    return
    ...
    art_form = mo.ui.form(
        mo.ui.dictionary({
            "palette":    mo.ui.dropdown(
                              options=["NCA Blue-Red", "Ocean-Earth", "Neon", "Plasma", "Viridis", "Twilight"],
                              value="NCA Blue-Red", label="Colour palette"),
            "n_colors":   mo.ui.slider(4, 15, value=10, show_value=True, label="Alphabet n"),
            "art_seed":   mo.ui.slider(0, 200, value=7, show_value=True, label="Rule seed"),
            "art_steps":  mo.ui.slider(10, 80, value=30, step=5, show_value=True, label="Steps"),
            "art_size":   mo.ui.slider(50, 300, value=120, step=10, show_value=True, label="Grid size"),
            "smoothing":  mo.ui.slider(0, 8, value=3, show_value=True, label="Smoothing radius"),
            "blur_sigma": mo.ui.slider(0.0, 4.0, value=0.0, step=0.5, show_value=True, label="Gaussian blur"),
        }, label="Art parameters"),
        submit_button_label="Generate art!",
        bordered=False,
    )
    art_form
    return (art_form,)
...
@app.cell(hide_code=True)
def _(art_form, hidden_ui):
    mo.stop(art_form.value is None, mo.md("👆 Set parameters and click **Generate art!**"))
    _v      = art_form.value
    _nc     = int(_v["n_colors"])
    _seed   = int(_v["art_seed"])
    _steps  = int(_v["art_steps"])
...
def _():
    ...
    Low $\rho$ → compressible → **simple structure**.
    High $\rho$ → incompressible → **complex dynamics**.

    This gives a continuous knob to match the target downstream domain.
    Orange dotted lines show the natural language corpora gzip targets from the paper.
    """)
    return
...
def _(n_seeds_ui, grid_ui, steps_ui, hidden_ui):
    ...
    complexity_dist = {}
    for _nc in mo.status.progress_bar(
        [2, 5, 10, 15], title="Sampling NCA complexity distributions …"
    ):
        _ratios = []
...
def _(complexity_dist, band_lo_ui, band_hi_ui):
    ...
    for _patch, _col in zip(_bp["boxes"], _colors):
        _patch.set_facecolor(_col); _patch.set_alpha(0.7)
...
# ─────────────────────────────────────────────────────────────────────────────
# PART 5 — WHY NCA RESEMBLES LANGUAGE
# ─────────────────────────────────────────────────────────────────────────────
...
def _():
    ...
    Dashed lines show natural language reference values. NCA hits these targets at $n \approx 8{-}10$.
    """)
    return
...
def _():
    ...
    but gives no tool to find the right settings for an arbitrary new domain.

    We introduce the **Complexity-Domain Fingerprint Matcher**:
    pick a target domain, tune the NCA parameters, and watch the radar chart
    converge. The fingerprint is a 4-dimensional profile:
    `(gzip_ratio, entropy, bigram_MI, zipf_β)`. A **match score** quantifies alignment.
    """)
    return
...
def _():
    ...
    matcher_form = mo.ui.form(
        mo.ui.dictionary({
            "domain":   mo.ui.dropdown(options=list(DOMAIN_TARGETS.keys()),
                                        value="Python Code", label="Target domain"),
            "match_n":  mo.ui.slider(2, 15,    value=5,    show_value=True, label="Alphabet n"),
            "match_h":  mo.ui.slider(8, 64,    value=16,   step=8, show_value=True, label="Hidden units"),
            "match_lo": mo.ui.slider(0.0, 0.8, value=0.3,  step=0.05, show_value=True, label="Band min ρ"),
            "match_hi": mo.ui.slider(0.2, 1.0, value=0.45, step=0.05, show_value=True, label="Band max ρ"),
        }, label="Fingerprint parameters"),
        submit_button_label="Run matcher",
        bordered=False,
    )
    mo.vstack([
        matcher_form,
        mo.md("_Tune parameters and click **Run matcher**. "
              "Goal: blue radar polygon overlaps orange target._"),
    ])
    return DOMAIN_TARGETS, matcher_form


@app.cell
def _(matcher_form, DOMAIN_TARGETS, grid_ui, steps_ui):
    mo.stop(matcher_form.value is None, mo.md("👆 Set parameters and click **Run matcher**"))
    _v           = matcher_form.value
    _target_name = _v["domain"]
    _target      = DOMAIN_TARGETS[_target_name]
    _n           = int(_v["match_n"])
    _hid         = int(_v["match_h"])
    _lo          = float(_v["match_lo"])
    _hi          = float(_v["match_hi"])
    _G           = grid_ui.value
...
def _():
    ...
    A gallery of trajectories across **complexity regimes** for $n = 10$ colours.
    Each row is a different randomly sampled rule — showing the breadth of the function class.
    """)
    return
...
def _():
    ...
    |---------|---------------|
    | 🧬 **164M NCA tokens beat 1.6B C4 tokens** | Structured synthetic data is more token-efficient than natural text |
    | 🎚 **Complexity is tunable** | Alphabet $n$ and gzip band give precise control over data statistics |
    | 🎯 **Domain-matched complexity matters** | Code → simpler NCA; math/text → richer NCA |
    | 🔄 **Attention carries the transferable prior** | Re-init experiments show attention weights transfer best |
    | 🔭 **Long-term vision** | Foundation models that acquire reasoning from fully synthetic data |
